...
movie_genre <- as.data.frame(movie_data$genres, stringAsFactors = F)
...
for(i in 1:nrow(movie_genre2)){
  for(col in 1:ncol(movie_genre2)){
    gen_col  = which(genre_mat1[1, ] == movie_genre2[i, col])
    genre_mat1[i+1, gen_col] <- 1
  }
}

genre_mat2 <- as.data.frame(genre_mat1[-1, ], stringAsFactors = F) #remove first row, which was the genre list
...
#to search a film by specifying the genre
...
#implement a single model in R project - Item based collaborative learning
recommendation_model$IBCF_realRatingMatrix$parameters


#similarity between users
similarity_mat <- similarity(ratingMatrix[1:4, ], method = "cosine", 
                             which = "users")
...
#table to display unique ratings
...
table_views <- table_views[order(table_views$views, decreasing = T), ] #sort by number of views
...
#visualize a bar plot for total number of views of top films
...
# Heatmap of Movie Ratings for first 20 rows & 25 columns 
image(ratingMatrix[1:20, 1:25], axes = FALSE, 
      main = "Heatmap of first 20 rows & 25 columns")
...
average_ratings <- rowMeans(movie_ratings)
 qplot(average_ratings, fill = I("steelblue"), color = I("red")) + 
...
sampled_data <- sample(x = c(TRUE, FALSE), size = nrow(movie_ratings), replace = T, prob = c(0.8, 0.2) )
...
#Build a recommender system on dataset 
